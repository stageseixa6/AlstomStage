<?php

/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

/**
 * Description of Controller
 *
 * @author travail
 */
class Model
{

    static $connections = array();
    public $conf = 'default';
    public $table = false;
    public $db;
    static $connect;

    function __construct()
    {
        //connexion à la base
        $conf = Conf::$databases[$this->conf];
        if (isset(Model::$connections[$this->conf])) {
            $this->db = Model::$connections[$this->conf];
        }

        try {
            $pdo = new PDO('mysql:host=' . $conf['host'] . ';dbname=' . $conf['database'] . ';', $conf['login'], $conf['password'], array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8')
            );
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
            Model::$connections[$this->conf] = $pdo;
            $this->db = Model::$connections[$this->conf];
            Model::$connect = $this->db;
        } catch (PDOException $e) {
            if (Conf::$debug >= 1) {
                die($e->getMessage());
            } else {
                die('impossible de se connecter à la base de données');
            }
        }
        //initialisation de variables
        if ($this->table === false) {
            $this->table = strtolower(get_class($this)) . 's';
        }
    }

    function find($req = '', $debug = FALSE, $ret = 'OBJ')
    {
        //si $ret !=OBJ on renverra un tableau associatif
        if ($ret == 'OBJ') {
            $par_ret = PDO::FETCH_OBJ;
        } else {
            $par_ret = PDO::FETCH_BOTH;
        }
        $sql = 'select ';
        //si la projection est renseignée on l'utilise sinon on met toutes les colonnes (*)
        if (isset($req['projection'])) {
            $sql .= $req['projection'] . ' ';
        } else {
            $sql .= '* ';
        }
        $sql .= ' from ' . $this->table . ' ';
        //construction de la condition

        if (isset($req['conditions'])) {
            $sql .= 'where ';
            if (!is_array($req['conditions'])) {
                $sql .= $req['conditions'];
            } else {
                $cond = array();
                foreach ($req['conditions'] as $k => $v) {
                    if (!is_numeric($v)) {
                        $v = $this->db->quote($v);
                    }

                    $cond[] = "$k = $v ";

                }
                $sql .= implode(' AND ', $cond);
            }
            $sql .= ' ';
        }

        // construction du like
        if (isset($req['like'])) {
            $sql .= 'where ';
            if (!is_array($req['like'])) {
                $sql .= $req['like'];
            } else {
                $lik = array();
                foreach ($req['like'] as $k => $v) {
                    if (!is_numeric($v)) {
                        $v = '%' . $v . '%';
                        $v = $this->db->quote($v);
                    }
                    $lik[] = "$k like $v";
                }
                $sql .= implode(' AND ', $lik);
            }
            $sql .= ' ';
        }
        // si le group by est renseigné on l'ajoute à la requete
        if (isset($req['groupby'])) {
            $sql .= 'group by ';

            $sql .= $req['groupby'];
        }
        // si le order by est renseigné on l'ajoute à la requete
        if (isset($req['orderby'])) {
            $sql .= 'order by ';

            $sql .= $req['orderby'];
        }

        if ($debug) {
            var_dump($sql);
        }
        try {
            $pre = $this->db->prepare($sql);
            $pre->execute();
            return $pre->fetchall($par_ret);
        } catch (PDOException $e) {
            if (Conf::$debug >= 1) {
                die($e->getMessage());
            } else {
                die('Problème pour executer la requête');
            }
        }
    }

    function findFirst($req, $debug = FALSE, $ret = 'OBJ')
    {
        return current($this->find($req, $debug, $ret));
    }

    function delete($req, $debug = FALSE)
    {
        $sql = 'delete from ' . $this->table . ' ';
        //construction de la condition

        if (isset($req['conditions'])) {
            $sql .= 'where ';
            if (!is_array($req['conditions'])) {
                $sql .= $req['conditions'];
            } else {
                $cond = array();
                foreach ($req['conditions'] as $k => $v) {
                    if (!is_numeric($v)) {
                        $v = $this->db->quote($v);
                    }
                    $cond[] = "$k = $v ";
                }
                $sql .= implode(' AND ', $cond);
            }
        }

        if ($debug) {
            var_dump($sql);
        }

        $info = null;
        try {
            $pre = $this->db->prepare($sql);
            $pre->execute();


//on traite l'exception dans le catch
        } catch (PDOException $e) {
            $info = $e->getMessage();
        }
        return $info;
    }

    function update($req, $debug = FALSE)
    {
        $info = null;
        $sql = 'update ' . $this->table . ' set ';
        //on récupère les données à mettre à jour dans $req['donnees'] et la clef primaire dans $req['cle']
        //on met des quotes aux chaines de caractères
        $cond = array();
        foreach ($req['donnees'] as $k => $v) {
            if (!is_numeric($v)) {
                $v = $this->db->quote($v);
            }
            $cond[] = "$k = $v ";
        }
        
        
        
        $sql .= implode(',', $cond);
        $sql .= ' where ';
        $cle = array();
        $conditions = array();
        if (isset($req['cle'])) {
            $conditions = $req['cle'];
        }
        if (isset($req['conditions'])) {
            $conditions = $req['conditions'];
            if (!is_array($req['conditions'])) {
                $sql .= $req['conditions'];
            } else {


                foreach ($conditions as $k => $v) {
                    if (!is_numeric($v)) {
                        $v = $this->db->quote($v);
                    }
                    $cle[] = "$k = $v ";
                }

                $sql .= implode(' AND ', $cle);
            }
        }
        if ($debug) {
            var_dump($sql);
        }
        try {
            $pre = $this->db->prepare($sql);

            $pre->execute();


//on traite l'exception dans le catch
        } catch (PDOException $e) {
            $info = $e->getMessage();
        }
        return $info;
    }

    //inserer une ligne avec une colonne en ai
    function insertAI($colonnes, $donnees, $debug = FALSE)
    {
        $sql = 'insert into ' . $this->table . ' ( ';
        $sql .= implode(',', $colonnes);
        $sql .= ') values(';
        //on récupère les données à mettre à jour dans $req['donnees'] et la clef primaire dans $req['cle']
        //on met des quotes aux chaines de caractères
        $cond = array();
        foreach ($donnees as $v) {
            if (!is_numeric($v)) {
                $v = $this->db->quote($v);
            }
            $cond[] = $v;
        }
      
        
        $sql .= implode(',', $cond);
        
        $sql .= ');';
        
//récupérer les erreurs

        if ($debug) {
            var_dump($sql);
        }
        try {
            $pre = $this->db->prepare($sql);
        } catch (PDOException $e) {
            if (Conf::$debug >= 1) {
                die($e->getMessage());
            } else {
                die("Ajout impossible, données invalides");
            }
        }
        try {
            $pre->execute();
        } catch (PDOException $e) {
            if (Conf::$debug >= 1) {
                die($e->getMessage());
            } else {
                die("Ajout impossible, données invalides");
            }
        }

        $tab = $this->db->query('SELECT LAST_INSERT_ID() as last_id');
        $tab1 = $tab->fetchALL(PDO::FETCH_ASSOC);
        $last_id = $tab1[0]['last_id'];
        return $last_id;


//on traite l'exception dans le catch
    }

//inserer une ligne avec une colonne en ai
    function insert($colonnes, $donnees, $debug = FALSE)
    {
        $info = null;
        $sql = 'insert into ' . $this->table . ' ( ';
        $sql .= implode(',', $colonnes);
        $sql .= ') values(';
        //on récupère les données à mettre à jour dans $req['donnees'] et la clef primaire dans $req['cle']
        //on met des quotes aux chaines de caractères
        $cond = array();
        foreach ($donnees as $v) {
            if (!is_numeric($v)) {
                $v = $this->db->quote($v);
            }
            $cond[] = $v;
        }
        $sql .= implode(',', $cond);

        $sql .= ');';

        if ($debug) {
            var_dump($sql);
        }
        try {
            $pre = $this->db->prepare($sql);
            $pre->execute();


//on traite l'exception dans le catch
        } catch (PDOException $e) {
            $info = $e->getMessage();
        }
        return $info;
    }

}
